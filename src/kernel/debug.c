...
void logf(const char* module, DebugLevel level, const char* fmt, ...)
{
    ...
    // fputs(g_LogSeverityColors[level], VFS_FD_STDOUT);    // set color depending on level
    // fprintf(VFS_FD_STDOUT, "[%s] ", module);             // write module
    // vfprintf(VFS_FD_STDOUT, fmt, args);                  // write text
    // fputs(g_ColorReset, VFS_FD_STDOUT);                  // reset format
