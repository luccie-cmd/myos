...
bool isNradix(long long n, int radix){
    // Handle special cases
    if (n == 0) return true;  // 0 is always valid
    if (radix < 2 || radix > 16) return false;  // Radix must be between 2 and 16
    ...
    while (n > 0) {
        ...
        // Check if the digit is valid for the given radix
        if (digit >= radix) {
            // If the digit is greater than or equal to the radix, it's invalid
            return false;
        }
    ...
    // All digits are valid for the given radix
    return true;
}
